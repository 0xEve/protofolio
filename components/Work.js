...
export default function Work() {
  ...
  return (
    <section id="work" className="max-w-[1240px] mx-auto px-6 md:px-10 py-24 md:py-36">
      ...
      <div className="space-y-20">
        {featured.map((p, i) => (
          <Reveal key={p.title}>
            ...
            >
              <div className={`overflow-hidden border border-line ${i % 2 === 1 ? "md:order-2" : ""}`}>
                <Cover index={i + 1} />
              </div>
              <div>
                ...
                <p className="mt-5 text-mut leading-relaxed max-w-lg">{p.description}</p>
                <div className="mt-6 flex flex-wrap gap-2">
                  {p.stack.map((t) => (
                    <span key={t} className="font-mono text-[10px] uppercase tracking-[0.15em] border border-line px-3 py-1.5 text-mut">
                      {t}
                    </span>
                  ))}
                </div>
                <span className="mt-8 inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.25em]">
                  View on GitHub
        ...
        <div className="mt-20 border-t border-line">
          {rest.map((p, i) => (
            <Reveal key={p.title} delay={i * 0.05}>
              <a href={p.link} target="_blank" rel="noreferrer" className="group grid grid-cols-[auto_1fr_auto] md:grid-cols-12 items-baseline gap-4 py-8 md:py-10 border-b border-line">
                <span className="md:col-span-1 font-mono text-xs text-mut">0{i + 3}</span>
                ...
                <span className="hidden md:block md:col-span-1 font-mono text-[11px] text-mut">{p.year}</span>
